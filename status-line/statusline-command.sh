...
# Session cost (API usage, always populated regardless of auth backend)
...
color_cost='\033[38;5;220m'    # gold — session cost
...
# Cost segment (session USD spend, always shown when present)
cost_part=""
if [ -n "$cost_usd" ]; then
  cost_fmt=$(printf '%.2f' "$cost_usd")
  cost_part="💰 ${color_cost}\$${cost_fmt}${reset}"
fi
