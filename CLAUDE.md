...
Personal Claude Code plugin marketplace. Structure: `plugins/dev-toolkit` (skills `grill-me`, `prove-it`, vendored `prompt-master` and `i-have-adhd`), `plugins/notify` (macOS notification hook), `rules/` (global rules copied into `~/.claude/rules`), `statusline/`, `.claude-plugin/marketplace.json`.
...
- Vendored code (e.g. `plugins/dev-toolkit/skills/prompt-master`, `plugins/dev-toolkit/skills/i-have-adhd`) keeps its own `LICENSE`/`README.md` under the original author — never merge into the repo-root `LICENSE`.
