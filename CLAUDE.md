...
Personal Claude Code plugin marketplace. Structure: `plugins/dev-toolkit` (skills `prove-it`, vendored `prompt-master`), `plugins/notify` (macOS notification hook), `rules/` (global rules copied into `~/.claude/rules`), `statusline/`, `.claude-plugin/marketplace.json`.
